...
';
  ...
  function product_theme($the_content) {
    global $post;
    ...
    //add thumbnail
		if ($settings['show_img']) {
			$content = mp_product_image( false, 'single' );
		} 
		
    //Title
    $content .= '<h3 class="mp_product_name">' . $post->post_title . '</h3>';

    //Adds content
    //$content .= '<div class="mp_product_content">'.$post->post_excerpt.'</div>' ;
    $user = get_user_by('id', $post->post_author) ;

    $content .= '<div class="mp_product_details right">';
    $content .= '<div class="left mp_product_reviews">';
    $content .= '<h4>Customer Reviews</h4>';

    //Adds rating
    if(function_exists('kk_star_ratings')) : $content .= kk_star_ratings($post->ID); endif;

    $content .= '<h4 class="review">Reviews ('.get_comment_pages_count($post->ID).')</h4>';
    $content .= '<a href="#tabs-4">Add a review</a>';

    $inventory = get_post_meta(get_the_ID(), 'mp_inventory', true) ;

    //Check number of products
    if (!empty($inventory)) {

      //If more than one, plural message
      if ($inventory[0] > 1) {
          $inventory_text = 'There are <span class="inventory">'.$inventory[0].'</span> products in stock.';
      }
      else{
          $inventory_text = 'There is only <span class="inventory red">'.$inventory[0].'</span> product in stock!!!';
      }

    }
    //Else no product message
    else{

        $inventory_text = 'This item is <span class="red">out of stock</span>.' ;

    }

    $content .= '<h4 class="availability">Availability</h4>';
    $content .= $inventory_text;

    $content .= '<h4 class="shipping">Shipping</h4>';
    $content .= 'Fast & Free!';

    $content .= '<h4 class="more-products">More products</h4>';
    $content .= 'See more products from <a href="'.home_url($path = '/', $scheme = null).'products-by/?seller='.$user->ID.'">'.$user->user_login.'</a>';

    $content .= '</div>' ;

    //Right hand side, prices
    $sale_price = get_post_meta(get_the_ID(), 'mp_sale_price', true) ;
    $price = get_post_meta(get_the_ID(), 'mp_price', true) ;

    //Test if there's a sale
    if($sale_price[0] != 0) {

      //Calculate saving
      $saving = ($price[0] - $sale_price[0]) ;
    ...
    if(isset($saving)){
      ...
      //Then current price is sale price
    ...
    $content .= '<div class="right mp_product_prices">' ;

    if(isset($saving)){
      $content .= '<span class="left">RRP:</span><span class="right">$'.$price[0].'</span>' ;
      $content .= '<span class="left clear">Saving:</span><span class="right">$'.$saving.'</span>' ;
    }
    $content .= '<div class="mp_product_current_price clear"><span class="left clear">Total price: </span><span class="right">$'.$current_price.'</span></div>' ;
    ...
    $content .= '</div></div>' ;
		...
		$content .= mp_category_list($post->ID, '<div class="mp_product_categories clear right">' . __( 'Categorized in ', 'mp' ), ', ', '</div><div class="clear"></div>');

    $details = get_post_meta(get_the_ID(), 'mp_details', true) ;
    $warranty = get_post_meta(get_the_ID(), 'mp_warranty', true) ;

    $comment_file = get_bloginfo($show = 'template_url', 'Display').'/comments.php' ;
    ...
    $content .= '<div id="tabs">
          ...
          <div id="tabs-4">
            '.comments_template($comments_file).'
          </div>
        </div>' ;
...
//var_dump($content) ;
    ...
    $content = $settings['msg']['product_list'];
    //$content .= mp_list_products(false);
    $taxonomy = '&'.$wp_query->query_vars['taxonomy'] . '=' . get_query_var($wp_query->query_vars['taxonomy']) ;
    //echo 1 ;
    $custom_query = 'post_type=product&post_status=publish'. $taxonomy ;
    $content .= mp_display_products($custom_query, 'grid') ;
    //$content .= get_posts_nav_link();
    return $content;
  }
    ...
    if (isset($_POST['empty_cart'])) { //empty cart contents
      ...
      if (defined('DOING_AJAX') && DOING_AJAX) {
    	...
    	<div id="mp_cart_actions_widget">
    	  <a class="mp_store_link" href="<?php echo bloginfo($show='home_url'); ?>"><?php _e('Browse Products &raquo;', 'mp') ?></a>
    	</div>
        <?php
        exit;
      }
