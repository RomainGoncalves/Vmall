...
function _mp_cart_login($echo = false) {
  ...
  if ( is_user_logged_in() || defined('MP_HIDE_LOGIN_OPTION') ) {
    ...
    $content .= '<fieldset><label>'.__('Username', 'mp').'</label>';
    $content .= '<input type="text" name="log" id="user_login" class="input" value="" size="20" />';
    $content .= '<label>'.__('Password', 'mp').'</label>';
    $content .= '<input type="password" name="pwd" id="user_pass" class="input" value="" size="20" /></fieldset>';
    ...
    if ($settings['force_login'])
    	$content .= apply_filters('register', '<a class="mp_cart_direct_checkout_link" href="'.site_url('signup/', 'login').'">'.__('Register Now To Checkout &raquo;', 'mp').'</a>');
		else
      $content .= '<a class="mp_cart_direct_checkout_link" href="' . mp_checkout_step_url('shipping') . '">' . __('Checkout Now &raquo;', 'mp') . '</a>';
		$content .= '</td>';
    $content .= '</tr>';
  ...
  if ($type == 'form') {
    ...
    if (count((array)$mp_gateway_active_plugins) == 1) {
      ...
      foreach ((array)$mp_gateway_active_plugins as $plugin) {
        $content .= '<input type="radio" class="mp_choose_gateway" name="mp_choose_gateway" id="'.$plugin->plugin_name.'" value="'.$plugin->plugin_name.'" '.checked($_SESSION['mp_payment_method'], $plugin->plugin_name, false).'/>';
        $content .= '<label for="'.$plugin->plugin_name.'">';
        if ($plugin->method_img_url) {

          //New icon for paypal
          if($plugin->plugin_name == 'paypal-express'){

            $content .= '<img src="' . get_bloginfo($show='template_directory').'/images/paypal.png' . '" alt="' . $plugin->public_name . '" />';
...
function mp_show_cart($context = '', $checkoutstep = null, $echo = true) {
  ...
  if ( mp_items_in_cart() || $checkoutstep == 'confirmation') {
    ...
    $content .= '<div id="mp_cart_actions_widget"><a class="mp_store_link" href="'.home_url($path = '/', $scheme = null).'">'.__('Browse Products &raquo;', 'mp').'</a></div>';
  }
...
function mp_category_list( $product_id = false, $before = '', $sep = ', ', $after = '' ) {
  ...
  if ( $terms )
    return $terms;
		...
		return __( '<div class="mp_product_categories clear right">Uncategorized</div>', 'mp' );
}
      ...
      $button .= '<span class="mp_no_stock">' . __('Out of Stock', 'mp') . '</span>';
	    ...
	    if ($context == 'list') {
	      ...
	      if ($settings['show_quantity'] && empty($meta["mp_file"])) {
	        $button .= '<span class="mp_quantity"><label class="left">' . __('Quantity:', 'mp') . '</label><input class="mp_quantity_field left clear" type="text" size="1" name="quantity" value="1" /></span>';
	      }
