...
Plugin URI: http://www.geekpress.fr/wordpress/extension/simple-pagination-pagination-avancee-wordpress-622/
...
class Simple_Pagination {
	...
	**/
	function deactivate() 
	{
		delete_option('_simple_pagination');	
	}
		...
		switch ( $type ) {
			...
			case 'checkbox':
				...
				echo '<fieldset>
						...
						<label for="' . $id . '">
							<input class="checkbox' . $field_class . '" type="checkbox" id="' . $id . '" name="_simple_pagination[' . $id . ']" value="1" ' . checked( $options[$id], 1, false ) . ' />	' . $desc . '					
						</label>
					</fieldset>';
			...
			case 'text':
			default:
		 		echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="_simple_pagination[' . $id . ']"  value="' . esc_attr( $options[$id] ) . '" />';
